# Basic SQL Query (MS SQL Server)

Basic SQL Queries for Iris Dataset - MS SQL Server Syntax
...
```sql
SELECT TOP 10 species, sepal_length, sepal_width 
FROM iris;
```
...
## MS SQL Server Specific Features

### Pagination with OFFSET/FETCH

Modern pagination (SQL Server 2012+)

**R:**

```r
iris %>% slice(11:20)  # rows 11-20
```
...
```r
iris %>% mutate(species_label = paste0(species, "_flower"))
```

```sql
-- SQL Server 2012+ CONCAT function
SELECT *,
    CONCAT(species, '_flower') as species_label
FROM iris;

-- Traditional concatenation
SELECT *,
    species + '_flower' as species_label
FROM iris;
```
...
```r
iris %>% mutate(length_text = as.character(sepal_length))
```

```sql
SELECT *,
    CAST(sepal_length AS VARCHAR(10)) as length_text,
    CAST(sepal_length AS NVARCHAR(10)) as length_unicode_text
FROM iris;
```
...
```r
iris %>% mutate(is_large = sepal_length > 6.0)
```

```sql
SELECT *,
    CASE WHEN sepal_length > 6.0 THEN 1 ELSE 0 END as is_large,
    CASE WHEN sepal_length > 6.0 THEN 'TRUE' ELSE 'FALSE' END as is_large_text
FROM iris;
```
